...
Every experiment in `results.tsv` contains:
...
ACE analyzes this across **all factory-managed projects** — not just the current one.
...
`factory/insights.py` scans all factory-managed projects (any directory with `.factory/results.tsv`), loads their experiment histories, and computes:
...
### The Vault as Long-Term Memory

When `FACTORY_VAULT_PATH` is configured, the Archivist writes structured notes to an Obsidian vault:

```
$FACTORY_VAULT_PATH/
├── 00-Factory/
│   ├── Dashboard.md             # Factory-wide status
│   ├── Patterns.md              # Recurring cross-project patterns
│   └── Decisions/               # Major decisions log
├── 10-Projects/{name}/
│   ├── {project}.md             # Project dashboard
│   └── Experiments/
│       └── {project}-NNN.md     # Individual experiment records
├── 20-Knowledge/
│   ├── Concepts/
│   └── Sources/                 # Research sources cited in experiments
└── MEMORY.md
```

The Researcher reads vault knowledge before doing web searches — if a topic is already covered by source notes in `20-Knowledge/Sources/`, it skips the web search and uses the vault's curated knowledge. This means the factory gets faster and more targeted as its vault grows.

Without a vault, the factory degrades gracefully: the Archivist writes to `.factory/archive/` instead, and cross-project insights still work through `results.tsv` files.
